...
export class CartService {
  ...
  async createCart(body: CartRequest): Promise<IResponse<Cart>> {
    let response: IResponse<Cart>;
    try {
  ...
  async addCartItem(body: CartItemRequest): Promise<IResponse<Cart>> {
    let response: IResponse<Cart>;
    try {
      const cart: Cart = await this.mealMeClientService.post<Cart>(
        'cart/add',
  ...
  async removeCartItems(body: CartItemRequest): Promise<IResponse<Cart>> {
    let response: IResponse<Cart>;
    try {
      const cart: Cart = await this.mealMeClientService.post<Cart>(
        'cart/remove',
  ...
  async retrieveCart(query: CartSearchRequest): Promise<IResponse<Cart>> {
    let response: IResponse<Cart>;
    try {
  ...
  async listCarts(query: CartSearchRequest): Promise<IResponse<Cart[]>> {
    let response: IResponse<Cart[]>;
    try {
