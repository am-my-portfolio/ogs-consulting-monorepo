...
export class SearchService {
  ...
  async searchStores(query: StoreSearchRequest): Promise<IResponse<Store[]>> {
    let response: IResponse<Store[]>;
    try {
  ...
  async getStoreDetails(storeIds: string): Promise<IResponse<Store[]>> {
    let response: IResponse<Store[]>;
    try {
      const storeList: StoreSearchResponse =
        await this.mealMeClientService.get<StoreSearchResponse>(
    ...
    try {
      ...
      if (query.store_id && !query.menu_id) {
        ...
        const inventoryResult =
          await this.inventoryService.getInventoryDetails(inventoryQuery);
