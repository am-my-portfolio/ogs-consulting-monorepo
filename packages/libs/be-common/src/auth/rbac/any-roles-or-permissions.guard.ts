...
import { TokenUtils, anyPermissionsMatch, anyRolesMatch } from '@am-ogs/fs-common';
import { AllPermissions, AllRoles } from '@am-ogs/types';

@Injectable()
export class AnyRolesOrPermissionsGuard implements CanActivate {
  private readonly logger = new Logger(AnyRolesOrPermissionsGuard.name);
  ...
  canActivate(
    ...
    if (is_user_token) {
      required_roles_found = anyRolesMatch(user, any_required_route_roles);
    }

    this.logger.debug({ required_roles_found, required_permissions_found }),
      '[canActivate]';
