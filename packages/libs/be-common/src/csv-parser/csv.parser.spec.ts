...
import { CsvEntity } from './mocks/csv.entity';
import { CsvEntityRemaped } from './mocks/csv.entity.remaped';
...
describe('CsvParser', () => {
  ...
  beforeEach(async () => {
    ...
    csv_parser = app.get<CsvParser>(CsvParser);
  });

  describe('parse simple', () => {
    it('should return list of 2', async () => {
      const csv_stream = fs.createReadStream(
        __dirname + '/../tests/simple.csv',
      );
      const entities = await csv_parser.parse(csv_stream, CsvEntity);
    ...
    it('should return list of first entity', async () => {
      ...
      const entities = await csv_parser.parse(csv_stream, CsvEntity, 1);
      const csv1 = new CsvEntity({ foo: '1', bar: 'a' });
    ...
    it('should return list of first entity', async () => {
      ...
      const entities = await csv_parser.parse(csv_stream, CsvEntity, 1, 1);
      const csv2 = new CsvEntity({ foo: '2', bar: 'b' });
    ...
    it('should return list of 2 separated by commma', async () => {
      ...
      const entities = await csv_parser.parse(csv_stream, CsvEntity, 10, 1, {
        separator: ',',
      });
  ...
  describe('parse invalid', () => {
    it('should reject an error', async () => {
      ...
      await expect(
        csv_parser.parse(csv_stream, CsvEntity),
  ...
  describe('parse remaped', () => {
    it('should remap keys and cast type', async () => {
      const csv_stream = fs.createReadStream(
        __dirname + '/../tests/remaped.csv',
      );
      const entities = await csv_parser.parse(csv_stream, CsvEntityRemaped);
